...
def fase_te_resultats_reals(categoria, df_resultats):
    if df_resultats.empty: return False
    mapa = {
        "1rs grup": "Grup", "2ns grup": "Grup", "3rs grup": "Grup",
        "Vuitens": "Vuitens", "Quarts": "Quarts", "Semis": "Semis",
        "Finalistes": "Finalistes", "Campió": "Campió", "Resultat final": "Resultat Final",
        "MVP": "MVP", "Bota d'Or": "Jugador Pichichi"
    }
    columna = mapa.get(categoria)
    if not columna or columna not in df_resultats.columns: return False
    return not df_resultats[columna].replace("", pd.NA).dropna().empty
...
if jugador is not None:
    ...
    if not df_j.empty:
        ...
        # Taula Semàfor
        dades_estat = []
        for cat, punts in punts_dict.items():
            p = float(punts) if pd.notna(punts) else 0.0
            max_p = float(MAX_PUNTS_CATEGORIA.get(cat, 0))
            te_resultats = fase_te_resultats_reals(cat, df_resultats)

            if max_p == 0: estat = "⚪ Pendent config."
            elif p >= max_p: estat = "🟢 Complet"
            elif p > 0: estat = "🟠 A mitges"
            else: estat = "🔴 Fallat" if te_resultats else "⚪ Pendent jugar"
            dades_estat.append({"Categoria": cat, "Obtinguts": p, "Màxims": max_p, "Estat": estat})

        def color_fila(row):
            if "🟢" in row["Estat"]: return ["background-color: rgba(40, 167, 69, 0.2); color: #155724; font-weight: 500;"] * len(row)
            if "🟠" in row["Estat"]: return ["background-color: rgba(255, 193, 7, 0.2); color: #856404; font-weight: 500;"] * len(row)
            if "🔴" in row["Estat"]: return ["background-color: rgba(220, 53, 69, 0.2); color: #721c24; font-weight: 500;"] * len(row)
            return [""] * len(row)

        c1.write("### 🚦 Estat de les prediccions")
        c1.dataframe(pd.DataFrame(dades_estat).style.apply(color_fila, axis=1).format({"Obtinguts": "{:.1f}", "Màxims": "{:.1f}"}), use_container_width=True, hide_index=True)
